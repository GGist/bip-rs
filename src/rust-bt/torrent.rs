...
impl<'a> Torrent<'a> {
    pub fn new<'a>(ben_torr: &'a BenVal) -> IoResult<Torrent<'a>> {
        ...
        let announce = match torr_dict.get("announce") {
        ...
        match torr_dict.get("announce-list") {
            Some(&List(ref n)) => {
                for i in n.iter() {
        ...
        let comment = match torr_dict.get("comment") {
        ...
        let created_by = match torr_dict.get("created by") {
            Some(&Bytes(ref n)) => Some(try!(n.container_as_str().ok_or(
        ...
        let creation_date = match torr_dict.get("creation date") {
            Some(&Bytes(ref n)) => Some(try!(n.container_as_str().ok_or(
        ...
        let info = match torr_dict.get("info") {
...
impl<'a> FileType<'a> {
    fn new<'a>(ben_info: &'a BenVal) -> IoResult<FileType<'a>> {
        ...
        if info_dict.contains_key("files") {
    ...
    fn multi_file(info_dict: &'a HashMap<String, BenVal>) -> IoResult<FileType<'a>> {
        ...
        for i in files.iter() {
            ...
            let path_list = match file.get("path") {
                Some(&List(ref n)) => n,
