...
impl BenVal {
    ...
    pub fn bytes<'a>(&'a self) -> Option<&'a [u8]> {
        match self {
            &BenVal::Bytes(ref n) => Some(n.slice_from(0)),
            _             => None
        }
    }
    ...
    pub fn str<'a>(&'a self) -> Option<&'a str> {
        match self {
            &BenVal::Bytes(ref n) => n.container_as_str(),
            _             => None
        }
    }
...
fn encode(val: &BenVal) -> Vec<u8> {
    match val {
        &BenVal::Int(ref n) => encode_int(n),
        &BenVal::Bytes(ref n) => encode_bytes(n.as_slice()),
        &BenVal::List(ref n) => encode_list(n),
        &BenVal::Dict(ref n) => encode_dict(n),
    }
}
...
fn decode(buf: &mut BufReader) -> BenResult<BenVal> {
    ...
    let ben_val = match curr_char {
        INT_START  => BenVal::Int(try!(decode_int(buf, BEN_END))),
        LIST_START => BenVal::List(try!(decode_list(buf))),
        DICT_START => BenVal::Dict(try!(decode_dict(buf))),
        BYTE_LEN_LOW...BYTE_LEN_HIGH => {
            // Back Stream Up So That First Digit Is Included
            try!(buf.seek(-1, SeekCur).or_else({ |e|
                Err(Slice(e.desc))
            }))
            BenVal::Bytes(try!(decode_bytes(buf)))
        },
...
fn decode_int(buf: &mut BufReader, delim: char) -> BenResult<i64> {
    ...
    let int_str = try!(int_bytes.container_as_str().ok_or(
        Slice("Could Not Parse u8 Array As str")
    ));
