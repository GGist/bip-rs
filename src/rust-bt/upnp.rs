...
static GENERIC_SEARCH: &'static str = "M-SEARCH * HTTP/1.1\r
...
static BASE_CONTENT_LENGTH: uint = 254; // Does Not Include Replacement Identifiers
...
impl ServiceDesc {
    ...
    //pub fn soap_request(action: &str, ) -> IoResult<> {
...
impl UPnPInterface {
    ...
    pub fn find_all(from_addr: SocketAddr) -> IoResult<Vec<UPnPInterface>> {
        let request = String::from_str(GENERIC_SEARCH).replace("{1}", "5").replace("{2}", "ssdp:all");
    ...
    pub fn find_services(from_addr: SocketAddr, name: &str, version: &str) -> IoResult<Vec<UPnPInterface>> {
        ...
        let request = String::from_str(GENERIC_SEARCH).replace("{1}", "5").replace("{2}", service_st.as_slice());
    ...
    pub fn find_devices(from_addr: SocketAddr, name: &str, version: &str) -> IoResult<Vec<UPnPInterface>> {
        ...
        let request = GENERIC_SEARCH.replace("{1}", "5").replace("{2}", device_st.as_slice());
    ...
    pub fn find_uuid(from_addr: SocketAddr, uuid: &str) -> IoResult<Option<UPnPInterface>> {
        ...
        let request = String::from_str(GENERIC_SEARCH).replace("{1}", "5").replace("{2}", uuid_st.as_slice());
...
fn parse_interfaces(payloads: Vec<String>) -> IoResult<Vec<UPnPInterface>> {
    let st_regex: Regex = regex!(r"ST: *([^\r\n]+)");
    let usn_regex: Regex = regex!(r"USN: *([^\r\n]+)");
    let location_regex: Regex = regex!(r"Location: *([^\r\n]+)");
...
fn send_search(from_addr: SocketAddr, timeout: uint, request: &str) -> IoResult<Vec<String>> {
    ...
    udp_sock.send_to(request.as_bytes(), DISCOVERY_ADDR);
