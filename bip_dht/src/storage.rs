...
impl AnnounceStorage {
    ...
    pub fn new() -> AnnounceStorage {
        AnnounceStorage{ storage: HashMap::new(), expires: Vec::new() }
    }
    ...
    pub fn find_items<F>(&mut self, info_hash: &InfoHash, item_func: F) where F: FnMut(SocketAddr) {
        self.find(info_hash, item_func, UTC::now())
    }
    
    fn find<F>(&mut self, info_hash: &InfoHash, mut item_func: F, curr_time: DateTime<UTC>) where F: FnMut(SocketAddr) {
        // Clear out any old contacts that we have stored
        self.remove_expired_items(curr_time);
        ...
        match (already_in_list, self.expires.len() < MAX_ITEMS_STORED) {
            (false, true) => {
                ...
                match self.storage.entry(item_info_hash) {
                    ...
                    Entry::Vacant(vac)       => { vac.insert(vec![item]); }
                };
...
impl ItemExpiration {
    pub fn new(info_hash: InfoHash, address: SocketAddr) -> ItemExpiration {
        ItemExpiration{ address: address, inserted: UTC::now(), info_hash: info_hash }
    }
