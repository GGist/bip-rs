...
impl TableBootstrap {
    pub fn new<I>(table_id: NodeId, id_generator: MIDGenerator, nodes: Vec<SocketAddr>, routers: I)
        -> TableBootstrap where I: Iterator<Item=SocketAddr> {
    ...
    pub fn start_bootstrap<H>(&mut self, out: &SyncSender<(Vec<u8>, SocketAddr)>, event_loop: &mut EventLoop<DhtHandler<H>>)
        -> BootstrapStatus where H: Handshaker {
    ...
    pub fn recv_response<'a, H>(&mut self, trans_id: &TransactionID, table: &RoutingTable, out: &SyncSender<(Vec<u8>, SocketAddr)>,
        event_loop: &mut EventLoop<DhtHandler<H>>) -> BootstrapStatus where H: Handshaker {
        // Process the message transaction id
        let timeout = if let Some(t) = self.active_messages.get(trans_id) {
            *t
        } else {
            warn!("bip_dht: Received expired/unsolicited node response for an active table bootstrap...");
    ...
    pub fn recv_timeout<H>(&mut self, trans_id: &TransactionID, table: &RoutingTable, out: &SyncSender<(Vec<u8>, SocketAddr)>,
        ...
        if self.active_messages.remove(trans_id).is_none() {
            warn!("bip_dht: Received expired/unsolicited node timeout for an active table bootstrap...");
            return self.current_bootstrap_status()
        }
    ...
    fn bootstrap_next_bucket<H>(&mut self, table: &RoutingTable, out: &SyncSender<(Vec<u8>, SocketAddr)>,
        ...
        if self.curr_bootstrap_bucket == 0 || self.curr_bootstrap_bucket == 1 {
            ...
            let iter = percent_25_bucket.chain(percent_50_bucket).chain(percent_100_bucket).filter(|n| n.status() == NodeStatus::Questionable);
            
            self.send_bootstrap_requests(iter, target_id, table, out, event_loop)
        }
    }
    
    fn send_bootstrap_requests<'a, H, I>(&mut self, nodes: I, target_id: NodeId, table: &RoutingTable, out: &SyncSender<(Vec<u8>, SocketAddr)>,
        event_loop: &mut EventLoop<DhtHandler<H>>) -> BootstrapStatus where I: Iterator<Item=&'a Node>, H: Handshaker {
