...
/// Compares the given ipv4 address against the given node id to see if the node
/// id is valid.
...
fn is_security_compliant_ipv4_exempt(addr: Ipv4Addr) -> bool {
    // TODO: Since we are not using this module yet, we dont have to use the ip
    // feature gate which is not stable yet.

    false
    // addr.is_loopback() || addr.is_private() || addr.is_link_local()
}

/// Compares the given masked ip (v4 or v6) against the given node id to see if
/// the node if is valid.
///
/// Also, LOL, the spec is so confusing when it comes to this and mixes variable
/// naming multiple times. If you understand how to generate a compliant id,
/// essentially assume the id is legit, take the rand variable which should be
/// the last byte of the node id, and then basically do what we would do when
/// generating an id (aside from generating filler random numbers which would be
/// wasteful here).
fn is_compliant_addr(masked_ip_be: u64, num_octets: usize, id: NodeId) -> bool {
    ...
    // Official spec says to store the rand_masked_ip in a 64 bit integer (8 byte
    // array) and hash the result, however, the rasterbar spec says to store
    // them in a 32 bit integer (4 byte array). We are performing the latter as
    // it seems to give us correct results for the different test vectors. We
    // will however store 8 octet addresses in an 8 byte array.

    // TODO: Not sure if this checksum uses a constant internally that depends on
    // endiannes of computer (this sentence is most likely stupid in more than
    // one way).
...
/// Compares the result from the crc32c function against the first 21 bits of
/// the node id.
///
/// We dont have to check the last byte of the node id since we used that byte
/// to generate the crc32c_result.
