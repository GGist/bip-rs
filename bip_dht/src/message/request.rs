...
impl<'a> RequestType<'a> {
    pub fn from_parts(
        ...
        match rqst_type {
            ...
            FIND_NODE_TYPE_KEY => {
                let find_node_rqst =
                    FindNodeRequest::from_parts(rqst_root, trans_id, message::TARGET_ID_KEY)?;
            ...
            unknown => {
                if let Some(target_key) = forward_compatible_find_node(rqst_root) {
                    let find_node_rqst =
                        FindNodeRequest::from_parts(rqst_root, trans_id, target_key)?;
                    Ok(RequestType::FindNode(find_node_rqst))
                } else {
...
///
/// Treat unsupported messages with either a target id key or info hash key as
/// find node messages.
