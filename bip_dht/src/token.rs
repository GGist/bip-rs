...
/// We will partially follow the bittorrent implementation for issuing tokens to
/// nodes, the secret will change every 10 minutes and tokens up to 10 minutes
/// old will be accepted. This is in contrast with the bittorrent implementation
/// where the secret changes every 5 minutes and tokens up to 10 minutes old are
/// accepted. Updating of the token will take place lazily. However, with our
/// implementation we are not going to store tokens that we have issued,
/// instead, store the secret and check if the token they gave us is valid for
/// the current or last secret. This is technically not what we want, but it
/// will have essentially the same result when we assume that nobody other than
/// us knows the secret.

/// With this scheme we can guarantee that the minimum amount of time a token
/// can be valid for is the maximum amount of time a token is valid for in
/// bittorrent in order to provide interop. Since we arent storing the tokens we
/// generate (which is awesome) we CANT track how long each individual token has
/// been checked out from the store and so each token is valid for some time
...
impl TokenStore {
    pub fn new() -> TokenStore {
        // We cant just use a placeholder for the last secret as that would allow
        // external nodes to exploit recently started dhts. Instead, just
        // generate another placeholder secret for the last secret with the
        // assumption that we wont get a valid announce under that secret. We
        // could go the option route but that isnt as clean.
...
/// Since we are lazily generating tokens, more than one interval could have
/// passed since we last generated a token in which case our last secret AND
/// current secret could be invalid.
///
/// Returns the number of intervals that have passed since the last refresh
/// time.
