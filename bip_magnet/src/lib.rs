...
&xs=dchub://example.org";
        ...
        assert_eq!(link.address_tracker, vec![
            "udp://tracker.openbittorrent.com:80/announce"
        ]);
        assert_eq!(link.acceptable_source, vec!["http://download.wikimedia.org/mediawiki/1.15/mediawiki-1.15.1.tar.gz"]);
        assert_eq!(link.exact_source, vec![
    ...
    #[test]
    fn test_tpb() {
        let url = "magnet:?xt=urn:btih:d9be6909325d28912f400fcb324005dd5861e49f&dn=Crunchbang+GNU%2FLinux+-+AMD64+ISO&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A80&tr=udp%3A%2F%2Fopen.demonii.com%3A1337&tr=udp%3A%2F%2Ftracker.coppersurfer.tk%3A6969&tr=udp%3A%2F%2Fexodus.desync.com%3A6969";
        let link = ::MagnetLink::parse(url).unwrap();

        let expected_info_hash = [
            0xd9, 0xbe, 0x69, 0x09, 0x32,
            0x5d, 0x28, 0x91, 0x2f, 0x40,
            0x0f, 0xcb, 0x32, 0x40, 0x05,
            0xdd, 0x58, 0x61, 0xe4, 0x9f
        ];
        assert_eq!(link.get_info_hash(), Some(ShaHash::from_hash(&expected_info_hash[..]).unwrap()));
