...
impl BDecodeOpt {
    ...
    /// Whether or not an error should be thrown for out of order dictionary
    /// keys.
    pub fn check_key_sort(&self) -> bool {
        self.check_key_sort
    }

    /// Whether or not we enforce that the decoded bencode must make up all of
    /// the input bytes or not.
    ///
    /// It may be useful to disable this if for example, the input bencode is
    /// prepended to some payload and you would like to disassociate it. In
    /// this case, to find where the rest of the payload starts that wasn't
    /// decoded, get the bencode buffer, and call len().
