        ...
        .and_then(move |framed| {
            timer
                ...
                .and_then(move |(msg, framed)| {
                    ...
                    // Check that it responds with the same hash and protocol, also check our
                    // filters
                    if remote_hash != hash
                        || remote_prot != prot
...
mod tests {
    ...
    fn positive_initiate_handshake() {
        ...
        // Setup our buffer so that the first portion is zeroed out (so our function can
        // write to it), and the second half is our serialized message (so our
        // function can read from it).
    ...
    fn positive_complete_handshake() {
        ...
        // Setup our buffer so that the second portion is zeroed out (so our function
        // can write to it), and the first half is our serialized message (so
        // our function can read from it).
        let mut writer = Cursor::new(vec![0u8; remote_message.write_len() * 2]);
