...
use bittorrent::{BTPeer, MioSender};
...
pub fn spawn_handshaker<S, T>(send: S,
                              listen: SocketAddr,
                              pid: PeerId,
                              protocol: &'static str,
          ...
          T: Send + 'static + From<BTPeer>
{
    ...
    eloop_config.notify_capacity(MAX_ELOOP_MESSAGE_CAPACITY)
          ...
          T: Send
{
    pub fn new(listener: TcpListener,
        ...
        HandshakeHandler {
            ...
            listener: listener,
            msg_ack: msg_ack,
            slab: Slab::new_starting_at(SLAB_START_TOKEN, MAX_CONCURRENT_CONNECTIONS),
            send: send,
            our_protocol: protocol,
            _unused: PhantomData,
        }
    }
}
          ...
          T: Send + From<BTPeer>
{
    ...
    fn ready(&mut self, event_loop: &mut EventLoop<HandshakeHandler<S, T>>, token: Token, events: EventSet) {
            ...
            while let Ok(Some((stream, _))) = accept_result {
                ...
                self.slab.vacant_entry().and_then(|entry| {
                    let timeout = event_loop.timeout_ms(entry.index(), MAX_READ_TIMEOUT_MILLIS)
                                            .expect("bip_handshake: Failed To Set Timeout For \
                                                     Read");
    ...
    fn notify(&mut self, event_loop: &mut EventLoop<HandshakeHandler<S, T>>, msg: Task<T>) {
        self.msg_ack.ack();
        ...
        match msg {
            Task::Connect(expect_pid, hash, addr) => {
                ...
                self.slab
                    .vacant_entry()
                    .and_then(|entry| TcpStream::connect(&addr).ok().map(|stream| (stream, entry)))
                    .and_then(|(stream, entry)| {
    ...
    fn timeout(&mut self, event_loop: &mut EventLoop<HandshakeHandler<S, T>>, timeout: InnerToken) {
...
impl Connection {
    pub fn new_initiate(stream: TcpStream,
