...
fn positive_custom_protocol() {
    ...
    // Create two handshakers to connect to each other and specify a custom protocol
    let mut handshaker_one = BTHandshaker::with_protocol::<MockProtocol>(m_send.clone(), "127.0.0.1:0".parse().unwrap(), pid_one, context.clone(), "NotTheBittorrentProtocol").unwrap();
    let handshaker_two = BTHandshaker::with_protocol::<MockProtocol>(m_send, "127.0.0.1:0".parse().unwrap(), pid_two, context, "NotTheBittorrentProtocol").unwrap();
