...
#[test]
fn negative_initiate_wrong_hash() {
    ...
    // Make sure we trigger a wrong hash on the initiate (so the initiater, handshaker one, is not interested but handshaker two is)
    ...
    // Assert that none of the handshakers finished
