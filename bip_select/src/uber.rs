...
trait DiscoveryTrait: ExtendedListener + Sink<SinkItem=IDiscoveryMessage,SinkError=DiscoveryError> + Stream<Item=ODiscoveryMessage,Error=DiscoveryError> {}
impl<T> DiscoveryTrait for T where T: ExtendedListener + Sink<SinkItem=IDiscoveryMessage,SinkError=DiscoveryError> + Stream<Item=ODiscoveryMessage,Error=DiscoveryError> {}
...
impl UberModuleBuilder {
    ...
    pub fn new() -> UberModuleBuilder {
        UberModuleBuilder{ discovery: Vec::new(), ext_builder: None }
    }
    ...
    pub fn with_discovery_module<T>(mut self, module: T) -> UberModuleBuilder
        where T: ExtendedListener +
                 ...
                 'static {
        self.discovery.push(Box::new(module) as Box<DiscoveryTrait<SinkItem=IDiscoveryMessage,SinkError=DiscoveryError,Item=ODiscoveryMessage,Error=DiscoveryError>>);
        self
    }
...
impl UberModule {
    ...
    pub fn from_builder(builder: UberModuleBuilder) -> UberModule {
        UberModule{ discovery: builder.discovery, extended: builder.ext_builder.map(|builder| ExtendedModule::new(builder)),
            last_sink_state: None, last_stream_state: None }
    }
    ...
    fn loop_states<G, A, L, R, E>(&mut self, is_sink: bool, init: Result<R, E>, get_next_state: G, assign_state: A, logic: L) -> Result<R, E>
        where G: Fn(&UberModule) -> Option<ModuleState>,
        ...
        let mut should_continue = result.as_ref().map(|async| (is_sink && async.is_ready()) || (is_stream && !async.is_ready())).unwrap_or(false);
        ...
        while should_continue && opt_next_state.is_some() {
            ...
            should_continue = result.as_ref().map(|async| (is_sink && async.is_ready()) || (is_stream && !async.is_ready())).unwrap_or(false);
    ...
    fn start_sink_state(&mut self, message: &IUberMessage) -> StartSend<(), UberError> {
        self.loop_states(true, Ok(AsyncSink::Ready), |uber| { uber.next_state(uber.last_sink_state) }, |uber, state| { uber.last_sink_state = state; }, |uber, state| {
            match (state, message) {
                (ModuleState::Discovery(index), &IUberMessage::Control(ref control)) => {
                    uber.discovery[index].start_send(IDiscoveryMessage::Control(control.clone()))
                        .map(|async| async.map(|_| ()))
                        .map_err(|err| err.into())
                },
                (ModuleState::Discovery(index), &IUberMessage::Discovery(ref discovery)) => {
                    uber.discovery[index].start_send(discovery.clone())
                        .map(|async| async.map(|_| ()))
                        .map_err(|err| err.into())
                },
                (ModuleState::Extended, &IUberMessage::Control(ref control)) => {
                    ...
                    uber.extended.as_mut().map(|ext_module| {
    ...
    fn poll_sink_state(&mut self) -> Poll<(), UberError> {
        self.loop_states(true, Ok(Async::Ready(())), |uber| { uber.next_state(uber.last_sink_state) }, |uber, state| { uber.last_sink_state = state; }, |uber, state| {
            match state {
                ModuleState::Discovery(index) => {
                    uber.discovery[index].poll_complete()
                        .map_err(|err| err.into())
                },
    ...
    fn poll_stream_state(&mut self) -> Poll<Option<OUberMessage>, UberError> {
        self.loop_states(false, Ok(Async::NotReady), |uber| { uber.next_state(uber.last_stream_state) }, |uber, state| { uber.last_stream_state = state; }, |uber, state| {
            match state {
                ModuleState::Extended => {
                    uber.extended.as_mut().map(|ext_module| {
                        ext_module.poll()
                            .map(|async_opt_message| async_opt_message.map(|opt_message| opt_message.map(|message| OUberMessage::Extended(message))))
                            .map_err(|err| err.into())
                    }).unwrap_or(Ok(Async::Ready(None)))
                },
                ModuleState::Discovery(index) => {
                    uber.discovery[index].poll()
                        .map(|async_opt_message| async_opt_message.map(|opt_message| opt_message.map(|message| OUberMessage::Discovery(message))))
                        .map_err(|err| err.into())
                }
            }
        })
