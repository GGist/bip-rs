...
use bip_peer::messages::{ExtendedType, ExtendedMessage};
use futures::Async;
...
impl UtMetadataModule {
    ...
    pub fn new() -> UtMetadataModule {
        UtMetadataModule{ completed_map: HashMap::new(), pending_map: HashMap::new(),
            active_peers: HashMap::new(), active_requests: Vec::new(), peer_requests: VecDeque::new(),
            opt_sink: None, opt_stream: None }
    }
    ...
    fn add_torrent(&mut self, metainfo: Metainfo) -> StartSend<IDiscoveryMessage, DiscoveryError> {
        ...
        self.completed_map.insert(metainfo.info().info_hash(), info_bytes);
    ...
    fn add_peer(&mut self, info: PeerInfo, ext_info: &ExtendedPeerInfo) -> StartSend<IDiscoveryMessage, DiscoveryError> {
        let our_support = ext_info.our_message().and_then(|msg| msg.query_id(&ExtendedType::UtMetadata)).is_some();
        let they_support = ext_info.their_message().and_then(|msg| msg.query_id(&ExtendedType::UtMetadata)).is_some();
        ...
        match (our_support, they_support, opt_metadata_size) {
            (true, true, Some(metadata_size)) => {
                self.active_peers.entry(*info.hash())
                    .or_insert_with(|| ActivePeers{ peers: HashSet::new(), metadata_size: metadata_size })
                    .peers
                    .insert(info);
            },
            _                                 => ()
    ...
    fn recv_request(&mut self, info: PeerInfo, request: UtMetadataRequestMessage) -> StartSend<IDiscoveryMessage, DiscoveryError> {
        if self.peer_requests.len() == MAX_PEER_REQUESTS {
            Ok(AsyncSink::NotReady(IDiscoveryMessage::ReceivedUtMetadataMessage(info, UtMetadataMessage::Request(request))))
        } else {
            self.peer_requests.push_back(PeerRequest{ send_to: info, request: request });
    ...
    fn recv_data(&mut self, info: PeerInfo, data: UtMetadataDataMessage) -> StartSend<IDiscoveryMessage, DiscoveryError> {
        ...
        let opt_index = self.active_requests.iter()
        ...
        if let Some(index) = opt_index {
            ...
            if let Some(&mut Some(ref mut pending)) = self.pending_map.get_mut(&info.hash()) {
                ...
                pending.left -= 1;
                (&mut pending.bytes.as_mut_slice()[data_offset..]).write(data.data().as_ref()).unwrap();
            }
        }
    ...
    fn retrieve_completed_download(&mut self) -> Option<Result<ODiscoveryMessage, DiscoveryError>> {
        let opt_completed_hash = self.pending_map.iter()
    ...
    fn retrieve_piece_request(&mut self) -> Option<Result<ODiscoveryMessage, DiscoveryError>> {
        for (hash, opt_pending) in self.pending_map.iter_mut() {
            let has_ready_requests = opt_pending.as_ref().map(|pending| !pending.messages.is_empty()).unwrap_or(false);
            let has_active_peers = self.active_peers.get(hash)
            ...
            if has_ready_requests && has_active_peers {
                ...
                self.active_requests.push(generate_active_request(selected_message, *selected_peer));

                return Some(Ok(ODiscoveryMessage::SendUtMetadataMessage(*selected_peer, UtMetadataMessage::Request(selected_message))))
            }
        }
    ...
    fn initialize_pending(&mut self) -> bool {
        ...
        for (hash, opt_pending) in self.pending_map.iter_mut() {
            ...
            pending_tasks_available |= opt_pending.as_ref().map(|pending| !pending.messages.is_empty()).unwrap_or(false);
        }
    ...
    fn validate_downloaded(&mut self) -> bool {
        ...
        for (&expected_hash, opt_pending) in self.pending_map.iter_mut() {
            let should_reset = opt_pending.as_mut().map(|pending| {
                if pending.left == 0 {
...
fn generate_active_request(message: UtMetadataRequestMessage, peer: PeerInfo) -> ActiveRequest {
    ActiveRequest{ left: Duration::from_millis(REQUEST_TIMEOUT_MILLIS), message: message, sent_to: peer }
}
...
fn pending_info_from_metadata_size(metadata_size: i64) -> PendingInfo {
    ...
    PendingInfo{ messages: messages, left: num_pieces, bytes: bytes }
}
...
impl ExtendedListener for UtMetadataModule {
    ...
    fn on_update(&mut self, info: &PeerInfo, extended: &ExtendedPeerInfo) {
        self.add_peer(*info, extended).expect("bip_select: UtMetadataModule::on_update Failed To Add Peer...");
