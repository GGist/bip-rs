...
struct ServerDispatcher<H> where H: ServerHandler {
    handler: H
}

impl<H> ServerDispatcher<H> where H: ServerHandler {
    ...
    fn process_request<'a, 'b>(&mut self, provider: &mut Provider<'a, ServerDispatcher<H>>,
        request: TrackerRequest<'b>, addr: SocketAddr) {
    ...
    fn forward_connect<'a>(&mut self, provider: &mut Provider<'a, ServerDispatcher<H>>,
        trans_id: u32, addr: SocketAddr) {
...
fn write_response<'a, 'b, H>(provider: &mut Provider<'a, ServerDispatcher<H>>, response: TrackerResponse<'b>,
    addr: SocketAddr) where H: ServerHandler {
    provider.outgoing(|buffer| {
        ...
        } else { None } // TODO: Add Logging
    });
}

impl<H> Dispatcher for ServerDispatcher<H> where H: ServerHandler {
    type Timeout = ();
