...
pub fn parse_announce_list<B>(
    root_dict: &dyn BDictAccess<B::BKey, B>,
) -> Option<&dyn BListAccess<B>>
where
    B: BRefAccess<BType = B>,
{
    CONVERT
        .lookup_and_convert_list(root_dict, ANNOUNCE_LIST_KEY)
        .ok()
}
...
{
    CONVERT
        .lookup_and_convert_str(root_dict, ANNOUNCE_URL_KEY)
        .ok()
}
...
{
    CONVERT
        .lookup_and_convert_int(root_dict, CREATION_DATE_KEY)
        .ok()
}
...
{
    CONVERT
        .lookup_and_convert_str(root_dict, CREATED_BY_KEY)
        .ok()
}
...
{
    CONVERT
        .lookup_and_convert_int(info_dict, PRIVATE_KEY)
        .ok()
        .map(|p| p == 1)
}
...
pub fn parse_files_list<B>(
    info_dict: &dyn BDictAccess<B::BKey, B>,
) -> ParseResult<&dyn BListAccess<B>>
where
    B: BRefAccess<BType = B>,
{
...
{
    CONVERT
        .lookup_and_convert_bytes(info_or_file_dict, MD5SUM_KEY)
        .ok()
}

/// Parses the path list from the file dictionary.
pub fn parse_path_list<B>(
    file_dict: &dyn BDictAccess<B::BKey, B>,
) -> ParseResult<&dyn BListAccess<B>>
where
    B: BRefAccess<BType = B>,
{
