...
impl<P> PeerWireProtocol<P> {
    ...
    ///
    /// Important to note that nested protocol should follow the same message
    /// length format as the peer wire protocol. This means it should expect
    /// a 4 byte (`u32`) message length prefix. Nested protocols will NOT
    /// have their `bytes_needed` method called.
